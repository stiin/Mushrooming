...
function initMap() {
    ...
    selectInteractionCluster = new ol.interaction.Select({
        ...
        style: mushroomFeatureStyle //-> this for changing the style on feature click
    });
    ...
    selectInteractionHighlight = new ol.interaction.Select({
        ...
        style: highlightedMushroomFeatureStyle //-> this for changing the style on feature click
    });
    ...
    var onMapClickHandleClusterFeatures = function(event) {
        ...
        if (feature) {
            ...
            popupContent.innerHTML = "<b>Mushroom specie(s):</b> " + "<br><ul>";
            ...
            if (features.length > 10) {
                if (view.getZoom() == maxZoom) {
                    popupContent.innerHTML = "There are more than 10 mushroom findings registered at this particular spot. Right now the program can't handle this many findings at the same place. Sorry for the inconvenience!";
	 	    sidebar.close('info');
                } else {
	    ...
	    if (highlightFeature.values_.name == "getPoint") {
	   	...
	   	if (value.precision == 0) {precision = "N/A";} else {precision = value.precision;}
	        ...
	        return true;
            	...
            	// The HTML was MAGICALLY closing the <ul> with and UNEXPECTED </ul> WTF?!
    ...
    map.on('click', function(event) {
        ...
        if (onMapClickHandleHighlightedFeatures(event)) {
            return;
        }

        if (onMapClickHandleClusterFeatures(event)) {
            return;
        }
